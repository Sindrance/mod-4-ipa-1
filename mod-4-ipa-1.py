...
def tic_tac_toe(board):
    ...
    if len(set([board[i][i] for i in range(len(board))])) == 1:
        return (board1[0][0])
    elif len(set([board1[i][len(board)-i-1] for i in range(len(board))])) == 1:
...
def eta(first_stop, second_stop, route_map):
    ...
    # Replace `pass` with your code. 
    # Stay within the function. Only use the parameters as input. The function should return your answer.
    list=[i for i in route_map.keys()]
    for i in list:
        if i[0]==first_stop:
            time=(route_map[i]["travel_time_mins"])
            stop1_count=(list.index(i))+1
        if i[1]==second_stop:
            stop2_count=(list.index(i))
            time=time+(route_map[i]["travel_time_mins"])
    for i in range(stop2_count-stop1_count):
        time=time+route_map[list[stop1_count]]["travel_time_mins"]
    return time 